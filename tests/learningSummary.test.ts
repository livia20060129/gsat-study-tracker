...
test('summary page has one global week/month switch and no separate total-hours card', () => {
  ...
  assert.match(styles, /\.summary-day\.is-mood-unwell\{--day-core-color:hsl\(0 var\(--day-mood-saturation\) 73%\)\}/);
  assert.match(styles, /\.summary-day\.is-mood-tired\{--day-core-color:hsl\(30 var\(--day-mood-saturation\) 78%\)\}/);
  assert.match(styles, /\.summary-day\.is-mood-out\{--day-core-color:hsl\(44 var\(--day-mood-saturation\) 87%\)\}/);
  assert.match(styles, /background:var\(--day-core-color,var\(--day-time-color\)\)/);
  ...
  assert.match(runtime, /42 \+ 58 \* Math\.min\(1, totalMinutes \/ maxMinutes\)/);
  assert.match(runtime, /--day-mood-saturation:\$\{moodSaturation\}%/);
