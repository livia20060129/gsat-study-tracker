...
test('summary page has one global week/month switch and no separate total-hours card', () => {
  ...
  assert.match(runtime, /function summaryMoodColor\(mood: string, saturation: number\)/);
  assert.match(runtime, /身體不適'\) return `hsl\(0 \$\{saturation\}% 73%\)`/);
  assert.match(runtime, /較疲累'.*較疲倦'.*return `hsl\(30 \$\{saturation\}% 78%\)`/);
  assert.match(runtime, /外出'\) return `hsl\(44 \$\{saturation\}% 87%\)`/);
