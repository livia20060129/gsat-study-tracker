...
`study_records` 使用伺服器產生的 `revision`，所有更新透過具基準版本的 `upsert_study_record()` 完成。上傳前會先鎖定日期，合併各分頁、本機與雲端已有內容；空白值不會取代另一端已填資料。若仍遇到其他裝置同時寫入，系統會重新讀取、合併並重試一次，最後仍失敗才顯示衝突。
...
| 版本 | 重要更新 |
| --- | --- |
| v171.1.14 | 儲存前交叉比對多分頁、本機與雲端紀錄，保留雙方已填項目並避免空白資料覆蓋。 |
| v171.1.2 | Azar 英文文法 Calendar 頁碼範圍改為逐分項建立獨立項目，並相容有無「系列」的教材名稱。 |
| v171.1.1 | 新增「Azar英文文法（中階）」14 章、149 個分項及頁碼對照。 |
