...
async function cloudSignIn(){
 ...
 try{
  ...
  if(r.error)throw r.error;cloudSetMessage('登入成功，正在讀取雲端資料。',true);
...
async function cloudSignUp(){
 ...
 try{
  ...
  else cloudSetMessage('帳號已建立；若專案要求 Email 驗證，請先到信箱完成驗證後再登入。',true);
...
function mergeV123Value(dst,src){
 ...
 if(Array.isArray(src)){
  if(!Array.isArray(dst)||!dst.length)return cloneObj(src);
  return dst;
 }
 if(typeof src==='object'){
  if(!dst||typeof dst!=='object'||Array.isArray(dst))dst={};
  for(var k in src)if(Object.prototype.hasOwnProperty.call(src,k))dst[k]=mergeV123Value(dst[k],src[k]);
  return dst;
 }
 if(typeof src==='boolean')return src?true:!!dst;
 if(src==='')return dst===undefined||dst===null?'':dst;
...
function ensureSundayDeferred(rec,date){
 ...
 Object.keys(wanted).forEach(function(k){clean.push(wanted[k]);changed=true});
 rec.items=clean;
 return changed;
}
...
function ensureDailyPresets(rec,date){
 ...
 if(!Array.isArray(rec.items))rec.items=[];
 var defs=presetsForDate(date),allowed={},i,x,d,changed=false;
 ...
 for(i=0;i<defs.length;i++){
  ...
  if(/^(cal_(ace|writing|grammar|gujin|natural)_|gcal_)/.test(d.key||'')){
   ...
   if(/^cal_grammar_/.test(d.key||'')||(/^gcal_/.test(d.key||'')&&df.title==='英文文法總複習講義')){
    ...
    for(var dk2 in df)if(Object.prototype.hasOwnProperty.call(df,dk2)&&x.f[dk2]!==df[dk2]){x.f[dk2]=df[dk2];changed=true}
   }
  }
...
function dailyMessageForDate(date){
 ...
 return base+(calendarDefsForDate(date).length||CALENDAR_MATH_PLAN[date]?'｜已加入 Google Calendar 當日讀書排程。':'');
...
function renderScienceFields(x,reviewMode){
 ...
 if(isCalendarNaturalIntegration(x)){
  ...
  var ih='';
  if(f.calendarTopic)ih+='<div class="field" style="margin-top:10px"><label>Google Calendar 當日主題</label><div class="fixed-book-value">'+esc(f.calendarTopic)+'</div></div>';
 ...
 if(isCalendarNatural(x)&&f.calendarTopic)h+='<div class="field" style="margin-top:10px"><label>Google Calendar 當日主題</label><div class="fixed-book-value">'+esc(f.calendarTopic)+'</div></div>';
...
function findRecursive(list,target){
 ...
 for(var i=0;i<list.length;i++){var x=list[i];if(x&&x.id===target)return x;if(x&&x.f){var y=findRecursive(x.f.makeupEntries,target)||findRecursive(x.f.reviewEntries,target)||findRecursive(x.f.interactiveEntries,target)||findRecursive(x.f.calendarIntegrationEntries,target);if(y)return y}}return null;
}
