-- v171: server-controlled study-record revisions + Google Calendar sync storage.
...
-- Deliberately no client policies. OAuth tokens are only available to service-role
-- Edge Functions. The browser obtains connection status through the function API.
...
-- Preserve/ensure user-only read access. Edge Functions write using service role.
do $$
begin
  if not exists (
    select 1 from pg_policies
    where schemaname='public' and tablename='calendar_tasks'
      and policyname='Users can read own calendar tasks'
  ) then
...
create index if not exists calendar_tasks_user_date_idx
  on public.calendar_tasks(user_id, event_date);

create index if not exists calendar_tasks_user_updated_idx
  on public.calendar_tasks(user_id, event_updated_at);
