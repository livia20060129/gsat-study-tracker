...
export async function verifyOAuthState(state: string): Promise<{ userId: string }> {
  ...
  const expected = await hmac(payload, cfg.stateSecret);
...
async function fetchCalendarEvents(accessToken: string, calendarId: string): Promise<{
  events: GoogleEvent[];
  timeMinDate: string;
  timeMaxDate: string;
}> {
  const timeMin = new Date(Date.now() - 60 * 86400000);
  const timeMax = new Date(Date.now() + 240 * 86400000);
...
export async function syncCalendarForUser(admin: SupabaseClient, userId: string) {
  ...
  if (!connection) return { connected: false, synced: 0 };
  ...
  try {
    ...
    await admin
      .from('google_calendar_connections')
      .update({ sync_error: error instanceof Error ? error.message : String(error), updated_at: new Date().toISOString() })
      .eq('user_id', userId);
    throw error;
  }
}
